...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        val dao = DatabaseProvider.getDatabase(this).chatDao()
...
@Composable
fun PortfolioScreen(viewModel: ChatViewModel) {
        ...
        bottomBar = {
            Surface(modifier = Modifier.height(80.dp).fillMaxWidth(), color = MaterialTheme.colorScheme.surface, tonalElevation = 2.dp) {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceAround, verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                    val navItems = listOf("TERMINAL", "CHAT", "SYSTEMS", "SETTINGS")
                    navItems.forEach { item ->
                        Column(horizontalAlignment = androidx.compose.ui.Alignment.CenterHorizontally, modifier = Modifier.clickable { currentScreen = item }) {
                            Surface(modifier = Modifier.width(64.dp).height(32.dp), shape = androidx.compose.foundation.shape.CircleShape, color = if (currentScreen == item) MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surface) {
                                Box(contentAlignment = androidx.compose.ui.Alignment.Center) { Text(if (item == "TERMINAL") "⌂" else if (item == "CHAT") "💬" else if (item == "SYSTEMS") "⎔" else "⚙︎", fontSize = 20.sp) }
                            }
                            Text(item, fontSize = 10.sp, fontWeight = FontWeight.Bold, color = if (currentScreen == item) MaterialTheme.colorScheme.onSurface else MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                    }
                }
            }
                        ...
                        Surface(modifier = Modifier.fillMaxWidth(), shape = androidx.compose.foundation.shape.RoundedCornerShape(28.dp), color = MaterialTheme.colorScheme.primaryContainer) {
                            Column(modifier = Modifier.padding(20.dp)) {
                                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                                    Text("LIVE NODE", fontSize = 10.sp, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onPrimaryContainer, modifier = Modifier.padding(vertical = 4.dp, horizontal = 12.dp))
